function [maskFinal,cellCoors]=fretGetCellMasks(imForSeg0, objectRectangles, minSize, imCFP, imFRET, objective)
...
if isempty(minSize) && objective==40 %|| isempty(minSize)
    binning=round(2032/size(imForSeg0,1)); %CCD camera; 1040
    minSize=2000/binning/binning;
end

if isempty(minSize) && objective==20 %|| isempty(minSize)
    binning=round(2032/size(imForSeg0,1));
    minSize=500/binning/binning;
end

maskFinal=false(size(imForSeg0));
for i=1:length(objectRectangles)
    maskFinal=processSingleObjectRectangle(maskFinal,objectRectangles{i},imForSeg0,minSize);
end

% figure(1); imagesc(maskFinal);
...
imForSeg=imfilter(imForSeg,fspecial('gaussian',5,2),'symmetric');
imSumNorm=mat2gray(imForSeg, [prctile(imForSeg(:),5) prctile(imForSeg(:),99.5)]);
imSumNorm=imsharpenSean(imSumNorm,4,50); %Try enhancing the edges
